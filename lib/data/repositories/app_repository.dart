...
class AppRepository {
  ...
  try {
    // This will call mock API updateTimeEntry
    return await _apiService.updateTimeEntry(id, data);
  } catch (e) {
  ...
  // lib/data/repositories/app_repository.dart - ADD THESE METHODS TO YOUR EXISTING REPOSITORY

// ADD these new methods to your existing AppRepository class:

// NEW: Get today's tasks
Future<List<TaskModel>> getTodaysTasks({String? userId}) async {
  try {
    return await _apiService.getTodaysTasks(userId: userId);
  } catch (e) {
    throw Exception('Failed to get today\'s tasks: ${e.toString()}');
  }
}
...
Future<List<TaskModel>> getOverdueTasks({String? userId}) async {
  try {
    return await _apiService.getOverdueTasks(userId: userId);
  } catch (e) {
    throw Exception('Failed to get overdue tasks: ${e.toString()}');
  }
}

// NEW: Get urgent tasks
Future<List<TaskModel>> getUrgentTasks({String? userId}) async {
  try {
    return await _apiService.getUrgentTasks(userId: userId);
  } catch (e) {
    throw Exception('Failed to get urgent tasks: ${e.toString()}');
  }
}
...
Future<Map<String, dynamic>> getProjectStats(String projectId) async {
  try {
    ...
    throw Exception('Failed to get project stats: ${e.toString()}');
  }
}
...
Future<Map<String, dynamic>> getDashboardStats({String? userId}) async {
  try {
    return await _apiService.getDashboardStats(userId: userId);
  } catch (e) {
    throw Exception('Failed to get dashboard stats: ${e.toString()}');
  }
}
...
Future<List<Map<String, dynamic>>> getRecentActivity({String? userId, int limit = 10}) async {
  try {
    return await _apiService.getRecentActivity(userId: userId, limit: limit);
  } catch (e) {
    throw Exception('Failed to get recent activity: ${e.toString()}');
  }
}

// NEW: Get task distribution
Future<Map<String, int>> getTaskDistribution({String? projectId}) async {
  try {
    return await _apiService.getTaskDistribution(projectId: projectId);
  } catch (e) {
    throw Exception('Failed to get task distribution: ${e.toString()}');
  }
}
...
Future<Map<String, int>> getPriorityDistribution({String? projectId}) async {
  try {
    return await _apiService.getPriorityDistribution(projectId: projectId);
  } catch (e) {
    throw Exception('Failed to get priority distribution: ${e.toString()}');
  }
}
}
